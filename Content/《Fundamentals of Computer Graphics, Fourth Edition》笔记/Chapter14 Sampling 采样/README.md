...
蒙特卡洛积分的重要性采样告诉了我们可以用一些近似目标分布的已知分布产生随机点来优化采样的效果，但是计算机的伪随机数生成器实际上只能生成出符合均匀分布的随机点，这一节则是介绍如何从已知的概率密度函数中生成所需的符合其它分布的随机点。基本的方法有三种：逆变换法，接受-拒绝法，Metropolis法(简称M-H法)。
